...
| R7-10 阶段 2 行为系统接入 | 演示新增可选 `fsm` 配置段（enabled/definition/stimulus_window_seconds/player_near_distance）：启用时装配改用框架 `FsmDecisionMaker` + `PerceptionModule`（决策表 #35 on_tick 钩子），行为链"枪声→警戒→呼叫支援→搜寻→问候"由配置内 FSM 定义数据驱动；宿主注入 `player_distance`/`player_near` 旗标；`smoke_fsm.gd` 五链路断言；修复无头模式首帧视口 64×64 导致玩家被错误钳制的问题（clamp_size 改由配置注入，不读瞬时视口） |
| R7-11 多 NPC 场景 | `scene.npcs` 规格表（名字/出生点/精灵染色/种子/吼叫广播开关/独立 FSM 定义，fail-fast：名字唯一、FSM 必须启用）：演示装配多 Agent——每 NPC 独立 Agent + FSM + 感知模块 + 身体/气泡；宿主声学传播（shout_when_say 台词"呼叫支援"→ stimulus.shout）驱动连锁反应；`sample_town.json` 三 NPC 剧本（守卫巡逻/警戒/呼叫支援/搜寻/问候，支援兵隐蔽/响应集合点，平民闲逛/逃窜/问候）；`smoke_town.gd` 八链路断言（巡逻/警戒/逃窜/隐蔽/呼叫/响应/搜寻/问候）×3 PASS；单 NPC 旧模式全部冒烟回归无退化 |
| R7-12 演示效果增强（用户反馈） | ① NPC 区分：每 NPC 独立 SVG 造型（守卫盾牌/支援兵十字/平民草帽，spec.sprite）+ 头顶常驻名牌（spec 染色）；② 多次开枪无反应根因 = greet/respond 状态缺枪声中断迁移 → 全部反应后置状态补 heard_gunshot 中断（问候→警戒/逃窜、响应→隐蔽）；`smoke_town` 新增二次开枪三 NPC 重响应回归断言（11 链路 ×3 PASS）；③ 装饰地图（scene.map_enabled：草地/道路/建筑/树木，无碰撞——碰撞与寻路属阶段 3）；④ 修正 godot-cpp 4.5 头文件名 polygon2d.hpp（无下划线） |
...
| R10-7 行为推进修复（用户反馈"市民/支援兵行为太简单"后同轮回归） | 根因：到达标志生命周期竞态——FSM 每 tick 重发同一移动意图，Agent::execute→move_to/set_path 重置 arrival_pending_，先于 plan_paths_and_report 消费，move_done 丢失 → 支援兵卡在 respond、平民卡在 flee。修复：①到达消费+回投前置到 _process 中 system_.tick() 之前（consume_arrival → report_action_result("completed") 同帧派发），R10-4 注释同步；②守卫 greet 状态恢复 heard_gunshot→alert 打断转换（工作区覆盖版丢失，R7-12"问候/响应可被枪声打断"回归链路恢复）。验证：smoke_town 11 链路全 PASS |
| R10-8 演示交互修复（用户反馈"名牌英文/树可穿过/NPC 互穿/放箱子卡死"） | ①名牌：spec.label 中文 + 白字贴头顶（-90px，气泡上方不重叠）；②树木：树心注册为单单元障碍（draw_map 与 register_map_obstacles 共用 kTreePos 表），玩家与 NPC 均不可穿过；③动态占据碰撞——NPC blocked_check 与玩家 is_pixel_blocked 均按 kAgentRadius=0.35u 检查其他 NPC 与玩家位置（互不穿透），GodotBody 被挡时保持 moving_ 原地等待（障碍移开/重规划后继续，不再停死）；④place_obstacle 改沿最近移动方向在前方 2 单元处放置 2×2 木箱（GDScript 传 dir），候选方向前→顺时针→逆时针→反（避免把玩家围进死角），放置失败打印提示。验证：smoke_town 11 链路全 PASS + 探针实测（树像素阻塞 true、名牌中文、放置后玩家脚下可通行、平民逼近玩家停在 0.36u） |
| R10-9 放箱交互体验（窗口实测：按住 E 连续报"四周都被挡住"） | 根因：E 键 key repeat（60fps 每帧一次）→ 无冷却时瞬间把周围 4 方向 2×2 放满，后续 repeat 全部"无法放置"。修复：①place_obstacle 限频——Engine 进程帧间隔 ≥15 帧（≈0.25s@60fps）才允许放置；②放置搜索从固定 2 单元改为沿方向 2→20 单元找最近空位（跳过已放区域沿视线排布），不再原地放满即失败。验证：按住 E 模拟 120 帧 0 次失败；smoke 全 PASS |
| R10-10 放置/寻路体验修正（用户反馈"木箱位置太远/受阻不重规划/终点无目的地"） | ①木箱放置距离收敛：沿方向 2→4 单元（0.5~1.0 世界单位）内找空位，按 E 木箱总落在附近（实测前方 0.6u 阻塞、1.4u 畅通、脚下不堵）；②find_path 终点 fallback：目标 cell 被阻塞（木箱压终点）时在终点周围 10 单元内螺旋找最近可达单元作为替代终点——NPC 不再"到不了目标就什么都不做"，而是走到目标旁合理落点（实测平民 flee 终点被木箱压住仍到达目标旁并迁移 shelter 发抖）；③fallback 仅当目标 cell 确实阻塞时触发（start==goal 已就位语义不误触发，否则路径每帧抖动 move_done 永不触发）；④GodotBody::set_path 路径去重：同一路径重复注入（宿主每帧重规划）不重置到达标志/句柄。验证：smoke_town 11 链路全 PASS；探针实测平民 flee→shelter 迁移、守卫绕行不卡死 |
| R10-11 放置反馈修正（用户反馈"按 E 多次没反应/位置不在附近"） | ①移除 8 单元兜底：放置严格限 2→4 单元（0.5~1.0u），绝不放远（宁可提示无法放置）；②冷却 15→6 帧（≈0.1s）：只防 key repeat 每帧重复，人手快速连按（~8 次/秒）全部响应；③玩家头顶反馈气泡：放置成功显示"已放置木箱"（0.5s）、失败显示"四周都被挡住，无法放置"（0.9s）——窗口内可直接看到结果（此前失败仅打印控制台，玩家不可见）。验证：探针 5 连按全响应、气泡可见、smoke_town 11 链路全 PASS、门禁 5/5 |
| R10-12 NPC 相遇卡死修复（用户反馈"路径交叉碰面就卡死，没有重新规划"） | 根因：寻路只把静态障碍（建筑/树/木箱）当障碍，A* 路径会穿过其他 NPC 当前位置；两 NPC 相遇时 blocked_check 双方互等（moving_ 保持等待），但重新规划出的路径仍穿过对方 → 死锁。修复：新增 find_path_avoiding——寻路时临时把其他 NPC 与玩家的当前位置**膨胀为 3×3 单元（0.75u，覆盖 0.35u 半径）**阻塞为障碍，求路径后恢复；plan_paths_and_report 与 place_obstacle 重规划均改用动态避障寻路。验证：探针实测守卫 search 与平民 flee 路径交叉——此前守卫卡 4 秒等超时，现在 3 秒内绕行到达 search 目标、双方最小间距 >0.35u；smoke_town 11 链路全 PASS；门禁 5/5 |
| R10-13 木箱不显示修复（用户反馈"木箱怎么没有在画面上显示"） | 根因：place_obstacle 视觉方块像素位置少加网格 origin（世界原点偏移）——网格原点在 (-6.4,-3.6)（窗口中心），但木箱按 `cell×px_per_cell` 直接算像素，玩家 cell≈40 时像素 x=1640 超出 1280 窗口，木箱被画到窗口外（网格阻塞与 is_pixel_blocked 均正常，唯独视觉方块不可见）。修复：改用 `grid_.cell_to_world(px0,py0)`（含 origin 偏移的单元中心世界坐标）×scale + 窗口中心。验证：探针实测木箱位置 (1062,362) 在窗口内、visible=true；smoke_town 11 链路全 PASS；门禁 5/5 |
| R10-14 城镇布局与行为增强（用户反馈"支援兵鬼畜/场景简陋/房屋全矩形/NPC行为太简单"） | ①支援兵鬼畜根因：pace1 巡逻目标距出生点仅 0.6u，走几步就到达、频繁往复——拉远为往返巡逻 pace1(3.2,0.1)→pace2(0.9,-0.3)（间距 >2u），并新增花店 (3.6,0.9) 作为绕行障碍；②完整道路网：kRoads 表（横向主路 y≈0 + 南北辅路、纵向西/中/东街，非障碍）+ 主路/街道中线；③房屋多样化：墙体 + 梯形屋顶多边形 + 门 + 窗（不再全矩形）；池塘改椭圆多边形 + 涟漪；树木加密至 13 棵；④行为增强：守卫巡逻三段 walk1(-4,0)→walk2(0,0)→walk3(-2.6,2.6) 覆盖全镇、平民 wander1(西街)→market(市集棚门口)→browse(查看)→wander2(花店)。验证：探针实测守卫三段巡逻、支援兵往返巡逻不鬼畜、平民完整行为链；smoke_town 11 链路全 PASS；门禁 5/5 |
| R10-15 NPC 卡死修复与严格测试（用户反馈"随便一试 NPC 卡住不动"） | 排查发现两类卡死：①支援兵 pace2 目标 (0.9,-0.3) 落在建筑 (0.2,-0.6,0.8,1.2) 内部，fallback 替代终点在两 cell 间每帧跳变、set_path 每帧重置到达标志 → move_done 永不触发 → 卡死。修复：pace2 目标移至建筑东侧空地 (1.6,-0.4)；②问候循环——greet 只有 player_near=false 才退出，玩家站着时 NPC 永久问候；且 idle→greet 无时长门槛被 walk1(1s) 抢先。修复：greet 加 elapsed_ge 2.0 超时退出、idle→greet 门槛 0.5s（低于 walk1 的 1s）。**严格测试**：新增 smoke_stress.gd——4 场景 × 20s 卡死检测（正常生活流/单次枪声/连续放箱/玩家靠近平民+移动），任何 NPC 连续 12s 位置意图不变即失败；接入 check-gate.sh 第 5 步（单 NPC + town 连锁 + 80s 压力三连）。验证：压力冒烟 0 卡死、smoke_town 11 链路 PASS、门禁 5/5 |
| R10-16 视觉建模增强（用户反馈"建模太丑，虽然是测试也不能这么丑"） | ①4 个角色精灵全部重绘为 Q 版精致 SVG（大头小身 + 投影 + 高光 + 五官/腮红/服装细节）：守卫（蓝制服+头盔+徽章）、支援兵（白大褂+红十字+护士帽）、平民（草帽+斜挎包+微笑）、玩家（红斗篷+发带）；②场景装饰：草地纹理斑点（220 深浅小点）、道路路缘边线、建筑加烟囱+地基阴影、树木双层树冠（深色底+高光层）+树桩明暗、街角花坛（8 处 ×5 花）、主路两侧路灯（5×2）；③全部为纯视觉非障碍，与网格注册无冲突。验证：探针实测 466 ColorRect/35 Polygon/4 精灵纹理加载成功、画面采样 36 种颜色内容多样；smoke_town 11 链路 PASS；门禁 5/5（含 80s 压力冒烟） |
| R10-17 城镇布局合理化（用户反馈"房子跑哪去了，位置合理吗"） | 原布局多栋建筑压在马路中央（主路 y=0 上有两栋、辅路被市集棚/石屋/仓库压住）——布局不合理。修复：①全部 9 栋建筑移入道路围成的街区（北区木屋/市集棚/花店/旅店/教堂、南区石屋/仓库/民居/谷仓），道路全程畅通；②树木 15 棵全部重排到草带/街区（避开道路与建筑，脚本校验 cell 级无冲突）；③池塘移入南草带西端；④花坛/路灯同步避开道路；⑤NPC 目标点全部移到道路/空地（避开建筑边缘，脚本校验距建筑 ≥0.4u）；⑥多 NPC 交汇死锁加固——GodotBody 新增等待让步（被动态占据持续 1.2s 后不再互挡，静态障碍仍挡）+ plan_paths 同 cell 直接报告完成（目标精确停在终点 cell 时 move_done 正常触发）。验证：压力冒烟 4 场景×20s 0 卡死、smoke_town 11 链路 PASS（search 目标改 3.2,0.2 断言同步）、门禁 5/5 |
