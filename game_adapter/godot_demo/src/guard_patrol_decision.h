...
// 黑板 alarm=true 或（yield_on_player_seen 且玩家进入感知范围）时同样返回
// ready=false 让位给能力候选（惊吓/警戒/问候），完整演示"决策器 pending → 模块候选兜底"管线。
// 全部行为参数来自 DemoConfig（PatrolParams）。
...
public:
    explicit GuardPatrolDecision(PatrolParams params) : params_(std::move(params)) {
        current_ = 1;                       // 首个巡逻目标为回路第二点（先向上巡）
        phase_left_ = params_.walk_seconds; // 初始即进入 walk 阶段（巡逻先走后歇）
    }
    ...
    void from_json(const json& in) override; // 对称恢复；参数由构造注入
